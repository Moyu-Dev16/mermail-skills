...
| Longer description | Mermail packages Agent Skills plus a remote MCP server (`https://console.mermail.app/mcp`, Official Registry id `app.mermail/mcp`). Users create a workspace API key (`sk-proj-…`), set `MERMAIL_API_KEY`, and install this plugin. Includes 14 skills covering agent inbox provisioning and verification mail, inbox management, compose/send, workspace admin, task triage, mailbox-agent chat, Composio third-party connect/execute, scheduling/GTM/support agent workflows, and OAuth-scoped Agent Wallet / PayBox transfers. Destructive tools require MCP confirmation tokens. |
