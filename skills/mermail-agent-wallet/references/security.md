...
- Always call `get_paybox_connection` once before claiming PayBox tools are missing or asking to reconnect Mermail MCP. After a usable/`ACTIVE` probe, do not conclude missing tools from an incomplete `tools/list` and do not ask to refresh/reconnect MCP for that reason — attempt the live operation. Only if the probe call itself is missing or hard-fails, stop and ask for full-profile OAuth. Handoffs use `console_url` (or ask owner) — not “MCP tools missing.” Require owner OAuth specifically for connect/reauth or legacy Agent Wallet operations; never improvise another payment path.
