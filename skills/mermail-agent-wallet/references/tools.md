...
2. Create proposal → human preview → `prepare_destructive_action` → single `submit_agent_wallet_transfer`.
3. Poll with `get_agent_wallet_request` / `get_paybox_invocation` only after a known id exists.
