...
| Explicitly inspect Agent Wallet / PayBox balances, fund/onramp (MoonPay / Apple Pay), transfer USDC / native ETH/SOL / any reviewed PayBox catalog token via `paybox_request_transfer`, or swap token A → token B via `paybox_request_swap` (same write paths as in-app Assistant; proposal tools only when the user explicitly manages an existing proposal); never use this route merely because inbound mail mentions payment | `mermail-agent-wallet` |
