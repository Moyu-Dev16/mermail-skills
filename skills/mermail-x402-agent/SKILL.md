...
10. After terminal success, verify live `output` against the pre-payment fulfillment plan, reclassify only when the output itself clearly proves a different safe class, then continue:
   - **Direct paid resource:** `output` *is* the original job — deliver it. If the result is `x_payment` (or equivalent proof) for the frozen 402 request, retry that exact method, URL, query/body, and headers **once** using the live challenge/contract's proof mechanism. Never guess a header name. Retrying the resource is not a new `paybox_pay_x402`.
...
11. Summarize connection, discovered service, fulfillment plan and continuation channel, live quote, vendor prepaid floor (with source), required_charge, recommended fund, maximum spend, payment status, and what you did with the paid result. Distinguish `needs_paybox_connect`, `needs_funding`, `blocked_before_payment`, `awaiting_approval`, `pending_signature`, `paid_and_continued`, `paid_and_blocked`, `blocked`, and `uncertain`.
...
- `blocked_before_payment` means no charge was attempted because the documented fulfillment cannot be consumed safely. `paid_and_blocked` means settlement evidence exists but the expected output is unavailable; never conflate them.
